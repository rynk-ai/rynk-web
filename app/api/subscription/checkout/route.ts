...
export async function POST(request: NextRequest) {
  try {
    ...
    const polarCustomerId = await getOrCreateCustomer(polar, db, session.user.id, session.user.email, session.user.name)
...
async function getOrCreateCustomer(
  ...
  const user = await db.prepare('SELECT polarCustomerId FROM users WHERE id = ?')
    .bind(userId)
    .first()
  ...
  await db.prepare('UPDATE users SET polarCustomerId = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?')
