 ...
 * ToC Generation API
 * 
 * POST /api/learning/generate-toc - Generate course structure with academic sources
 */
...
Return ONLY the JSON, no other text.`
    ...
    try {
      ...
      console.error('[generate-toc] JSON parse error, attempting recovery...')
      ...
      try {
        ...
        if (unitsMatch) {
          ...
          console.warn('[generate-toc] Falling back to minimal course structure')
        } else {
          throw parseError
        }
      } catch {
    ...
    console.error('[/api/learning/generate-toc] Error:', error)
    return NextResponse.json(
