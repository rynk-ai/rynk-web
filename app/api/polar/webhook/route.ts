...
async function handleOrderPaid(db: D1Database, order: any) {
  ...
  const user = await db.prepare('SELECT id, credits FROM users WHERE polarCustomerId = ?')
    .bind(customerId)
    .first()
  ...
  if (productId === extraCreditsProductId) {
    ...
    await db.prepare('UPDATE users SET credits = credits + 500, updatedAt = CURRENT_TIMESTAMP WHERE id = ?')
      .bind(user.id)
      .run()
...
async function handleSubscriptionCreated(db: D1Database, subscription: any) {
  ...
  if (user) {
    ...
    await db.prepare(`
      ...
      WHERE id = ?
    `).bind(tier, customerId, subscription.id, credits, nextResetDate.toISOString(), user.id).run()
    
    console.log('✅ [Polar Webhook] Updated user subscription:', user.id, tier)
  } else {
    console.error('❌ [Polar Webhook] User not found for email:', customer.email)
  }
}
...
async function handleSubscriptionUpdated(db: D1Database, subscription: any) {
  ...
  `).bind(tier, subscription.status, subscription.id).run()
